...
function CourtDetailsBody() {
  ...
  const selectSlot = (e, slot) => {
    ...
    const newFilter = filteredTimings.filter(
  ...
  const removeSlot = (slotToRemove) => {
    setSelectedSlots((prevSelectedSlots) =>
      prevSelectedSlots.filter((slot) => slot.id !== slotToRemove.id)
    );

    setFilteredTimings((prevFilteredTimings) => [
      ...prevFilteredTimings,
      slotToRemove,
    ]);
  };
      ...
      {openTimeslot && (
        ...
        >
          <div className="time-slot-select-modal p-3">
            ...
            {calenderOpen && (
              <div className="calender-box">
                ...
                <div className="d-flex justify-content-end mt-2 gap-3 p-2">
                  <button
                    className="common-button bg-black text-white"
            ...
            <div
              className="range-label position-relative mt-3 p-2 text-primary"
              onClick={() => {
                setOpend(true);
              }}
            >
              ...
              {opend && (
                <ul className="slot-list">
                  {filteredTimings
                    ...
                    .sort((a, b) => a.id - b.id)
                    .map((slot) => (
                      <li key={slot.id} onClick={(e) => selectSlot(e, slot)}>
                        {slot.name}
                      </li>
                    ))}
                </ul>
              ...
              <div className="d-flex gap-2 mt-2">
                {selectedSlots.map((slot) => (
                  <div
                    className="selected-slot border border-1 rounded-2 px-2 py-1 mt-2 position-relative"
                    key={slot.id}
                  >
                    <span>{slot.name}</span>
                    <button
            ...
            <div className="d-flex justify-content-end mt-2 gap-3 p-2">
              ...
              <button className="common-button" onClick={createCourtSchedule}>
                Create
              </button>
            </div>
          </div>
      ...
      {bookingModal && (
        ...
        >
          <div className="time-slot-select-modal p-3 h-100 d-flex flex-column">
            ...
            <input
              type="date"
              className="p- px-2 mx-2 border rounded-1"
              value={selectedDate}
            ...
            <div className="d-flex justify-content-end mt-2 gap-3 p-2">
              ...
              <button className="common-button" onClick={initiatefunction}>
                Book
              </button>
            </div>
          </div>
